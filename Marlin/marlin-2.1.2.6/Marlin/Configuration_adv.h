...
#define CASE_LIGHT_ENABLE
  ...
  #define CASE_LIGHT_PIN PA1                 // Octopus BED output repurposed for lamp PWM via M355
  #define INVERT_CASE_LIGHT false             // Set true if Case Light is ON when pin is LOW
  #define CASE_LIGHT_DEFAULT_ON true          // Start with lamps on after Octopus boot
  #define CASE_LIGHT_DEFAULT_BRIGHTNESS 179   // Default lamp brightness (0-255, requires PWM pin)
...
#define HOMING_BUMP_MM      { 2, 2, 2, 2, 2, 2 }             // (linear=mm, rotational=°) Backoff from endstops after first bump
 ...
 */
   ...
   */
  #if HAS_STEALTHCHOP
    //#define STEALTHCHOP_XY  // Use spreadCycle for higher loaded-motion torque
   ...
   */
  #define CHOPPER_TIMING CHOPPER_DEFAULT_24V        // 24V motor power supply
...
/**
 * Hexapod spider SD file launcher.
 *
 * Use `M215` to list available codes and `M215 S<code>` to run one.
 * Example: `M215 S1` runs `/gcodes/input.txt`.
 */
#define SPIDER_SD_FILE_CODES
#if ENABLED(SPIDER_SD_FILE_CODES)
  #define SPIDER_SD_HOME_FILE "/gcodes/home.txt"
  #define SPIDER_SD_POS1_FILE "/gcodes/pos1.txt"
  #define SPIDER_SD_POS2_FILE "/gcodes/pos2.txt"
  #define SPIDER_SD_POS3_FILE "/gcodes/pos3.txt"
  #define SPIDER_SD_FILE_CODES_LIST \
    SPIDER_SD_FILE_CODE(1, "/gcodes/input.txt") \
    SPIDER_SD_FILE_CODE(2, "/gcodes/input2.txt") \
    SPIDER_SD_FILE_CODE(3, "/gcodes/input3.txt") \
    SPIDER_SD_FILE_CODE(4, "/gcodes/input4.txt") \
    SPIDER_SD_FILE_CODE(5, "/gcodes/input5.txt") \
    SPIDER_SD_FILE_CODE(6, "/gcodes/input6.txt") \
    SPIDER_SD_FILE_CODE(7, "/gcodes/input7.txt")
#endif

/**
 * Compatibility layer for simulator spider files copied directly to SD.
 *
 * Treats `@LOOP` as "jump back to the start of the current SD file"
 * and otherwise leaves `X Y Z A B C` commands unchanged.
 */
#define SPIDER_SD_FILE_COMPAT
