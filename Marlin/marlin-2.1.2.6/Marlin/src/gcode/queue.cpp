...
#if ENABLED(SPIDER_SD_FILE_COMPAT)

  static bool spider_sd_loop_directive(char * const line) {
    char *p = line;
    while (*p == ' ' || *p == '\t') ++p;
    if (*p == '@') ++p;
    if (!*p) return false;
    for (uint8_t i = 0; i < 4; ++i) {
      const char expected = "LOOP"[i];
      if (expected == '\0') break;
      if (!p[i] || toupper(static_cast<unsigned char>(p[i])) != expected) return false;
    }
    p += 4;
    return !*p || *p == ' ' || *p == '\t' || *p == ';';
  }

  static bool preprocess_spider_sd_command(char * const line) {
    if (spider_sd_loop_directive(line)) {
      card.setIndex(0);
      return true;
    }
...
void GCodeQueue::get_serial_commands() {
  ...
  inline void GCodeQueue::get_sdcard_commands() {
    ...
    int sd_count = 0;
    bool committed_spider_line = false;
    while (!ring_buffer.full() && !card.eof()) {
      ...
      if (is_eol || card_eof) {
        ...
        if (!process_line_done(sd_input_state, command.buffer, sd_count)) {
          ...
          #if ENABLED(SPIDER_SD_FILE_COMPAT)
            if (preprocess_spider_sd_command(command.buffer)) continue;
          #endif
          ...
          if (card.isInteractiveJob())
            committed_spider_line = true;
        ...
        if (committed_spider_line) break;
      }
      else
