...
bool CardReader::interactive_sd_job = false;
bool CardReader::interactive_sd_loop = false;
char CardReader::interactive_sd_path[MAXPATHNAMELENGTH], CardReader::filename[FILENAME_LENGTH], CardReader::longFilename[LONG_FILENAME_LENGTH];
...
void CardReader::configureInteractiveJob(const bool state, const bool loop/*=false*/, const char * const path/*=nullptr*/) {
  interactive_sd_job = state;
  interactive_sd_loop = state && loop;

  if (state && path) {
    strlcpy(interactive_sd_path, path, sizeof(interactive_sd_path));
    return;
  }

  interactive_sd_path[0] = '\0';
}
...
void CardReader::fileHasFinished() {
  const bool was_interactive = interactive_sd_job;
  const bool should_loop_interactive = isInteractiveLoopJob();
  char loop_path[MAXPATHNAMELENGTH] = { 0 };
  if (should_loop_interactive)
    strlcpy(loop_path, interactive_sd_path, sizeof(loop_path));
  file.close();
  #if HAS_MEDIA_SUBCALLS
  ...
  if (should_loop_interactive) {
    openFileRead(loop_path);
    configureInteractiveJob(true, true, loop_path);
    startOrResumeFilePrinting();
    SERIAL_ECHOPGM("Looping spider SD file: ");
    SERIAL_ECHOLN(loop_path);
    return;
  }
  ...
  if (was_interactive) {
    ...
    SERIAL_ECHOLNPGM("Spider SD file finished.");
    return;
  }
